"""Startup recovery for Modal polling threads.

On backend restart, in-flight Modal jobs still exist in DB but their monitor
threads are gone. This module reattaches monitors so failover/progress checks
continue exactly like fresh dispatches.
"""
...
class StartupRecovery:
    ...
    def recover(self) -> None:
        ...
        log.info("Modal recovery: reattaching %s in-flight monitor(s)", len(rows))
        for row in rows:
    ...
    def _recover_single(self, row: dict) -> None:
        ...
        if group_id and input_filename:
            blend_url = (
                ...
                f"/render-groups/{group_id}/input/{input_filename}"
            )
        elif input_filename:
            blend_url = (
                f"{self._cfg.public_backend_url}"
                f"/jobs/{job_id}/input/{input_filename}"
            )
        else:
            blend_url = ""
