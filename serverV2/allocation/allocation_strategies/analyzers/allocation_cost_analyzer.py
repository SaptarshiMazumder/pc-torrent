...
startup) with target prices to produce a ``AllocationCostEstimate``.  Used by
...
Smoke test: ``python -m serverV2.allocation.allocation_strategies.analyzers.allocation_cost_analyzer``
"""
...
class AllocationMixSlot:
    ...
    after Phase 1, so building a slot is one line at the call site::
        ...
        slot = AllocationMixSlot(target.render_speed, target.price_per_hour, frames)
    """
    render_speed: float
...
def estimate_cost_for_mix(
    ...
    *,
    ...
    ``mix`` is one ``AllocationMixSlot`` per target getting some share of frames.
    Empty mix → all-zero estimate (no division-by-zero, no NaNs).

    Each ``AllocationMixSlot`` is treated as one chunk.  Startup overhead is
    ...
    """
    if not mix:
        return AllocationCostEstimate(0.0, 0.0, 0.0, 0.0)
...
def _smoke() -> None:
    ...
    single_mix = [AllocationMixSlot(render_speed=1.0, price_per_hour=1.0, frames_assigned=60)]
    ...
    pair_speed = [
        AllocationMixSlot(2.0, 1.0, 30),   # fast — 30 frames at 0.5x time
        AllocationMixSlot(1.0, 1.0, 30),   # slow — 30 frames at 1.0x time
    ]
    ...
    pair_unbal = [
        AllocationMixSlot(1.0, 1.0, 10),
        AllocationMixSlot(1.0, 1.0, 50),
    ]
    ...
    pricey_mix = [AllocationMixSlot(1.0, 4.0, 60)]   # 4x price
    ...
    small = estimate_cost_for_mix(baseline, [AllocationMixSlot(1.0, 1.0, 10)])
    big = estimate_cost_for_mix(baseline, [AllocationMixSlot(1.0, 1.0, 100)])
    ...
    mixed_zero = [
        AllocationMixSlot(1.0, 1.0, 0),
        AllocationMixSlot(1.0, 1.0, 60),
    ]
    ...
    neg_mix = [AllocationMixSlot(1.0, -5.0, 60)]
