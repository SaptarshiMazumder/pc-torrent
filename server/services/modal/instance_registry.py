"""
ModalInstanceRegistry — thread-safe in-memory state store for active Modal jobs.

Mirrors the Vast InstanceRegistry pattern so the desktop UI can show
real-time monitoring cards for Modal jobs (status, progress, which
monitor check triggered, etc.).
"""
