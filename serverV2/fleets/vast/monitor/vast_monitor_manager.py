"""VastMonitorManager — thread manager for Vast instance monitors.
...
from serverV2.fleets.vast.monitor.vast_instance_monitor import VastInstanceMonitor
    ...
    def start_monitoring(
        ...
        *,
        ...
        if self._orchestrator is None:
            raise RuntimeError(
                "VastMonitorManager.set_orchestrator must be called before start_monitoring"
            )

        # Local de-dup: this instance already runs the monitor.  Skip
        # without touching Redis; the lock would just say "you already
        # own it".  Saves one round-trip on the fast path where dispatch
        # and a sweeper sweep collide on the same process.
        with self._monitors_lock:
            if job_id in self._monitors:
                return

        # Cross-instance dedup: at most one Cloud Run instance owns the
        # monitor for this job.  Lose -> someone else owns it; bail.
        ...
        vast_instance_id = int(provider_job_id)
        ...
        def _run_and_cleanup() -> None:
            try:
                ...
                # take over instantly if the monitor exited because the
                # job was reassigned (refresh returned False).  Best-
                # effort: if we already lost ownership the Lua release
                # is a no-op.
