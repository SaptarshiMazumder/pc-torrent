...
Parses the binary .blend format (Blender 2.8+) without requiring
Blender installed.  Handles gzip and zstd compressed files transparently.
...
def parse_blend_frame_range(file_data: bytes) -> dict:
    ...
    # Transparently decompress zstd (Blender 3.0+ default compression)
        ...
        try:
            ...
            try:
                ...
                # Content size unknown in frame header — use streaming fallback
                reader = ctx.stream_reader(io.BytesIO(file_data))
                chunks = []
                while True:
    ...
    for code, size, sdna_idx, count, bdata in _iter_blocks(file_data, pointer_size, endian):
        if code == b"SC\x00\x00" or code[:2] == b"SC":
            ...
            if sdna_idx != scene_idx and block_type_name != "Scene":
                continue
...
def _find_blend_in_zip(zf: zipfile.ZipFile) -> str:
    """Find the best .blend file candidate inside a ZipFile."""
    blend_names = [
    ...
    if not blend_names:
        ...
        logger.error("No .blend file in zip. Files found: %s", all_files[:20])
        raise BlendParseError(
            f"No .blend file found inside the zip archive. "
            f"Found {len(all_files)} files: {', '.join(all_files[:10])}"
        )
    # Prefer root-level .blend files over deeply nested ones
    blend_names.sort(key=lambda n: n.count("/"))
    logger.info("Found .blend candidates in zip: %s — using: %s", blend_names, blend_names[0])
...
def parse_blend_from_zip(zip_data: bytes) -> dict:
    ...
    try:
        with zipfile.ZipFile(io.BytesIO(zip_data)) as zf:
            blend_path = _find_blend_in_zip(zf)
            blend_data = zf.read(blend_path)
            logger.info("Extracted '%s' from zip (%d bytes)", blend_path, len(blend_data))
            if len(blend_data) < 12:
                raise BlendParseError(f"Extracted .blend file '{blend_path}' is too small ({len(blend_data)} bytes)")
            return parse_blend_frame_range(blend_data)
    except zipfile.BadZipFile:
        raise BlendParseError("Invalid zip archive — file may be corrupted or not a real zip")
    except BlendParseError:
        raise
    except Exception as e:
        logger.exception("Unexpected error extracting .blend from zip")
        raise BlendParseError(f"Failed to extract .blend from zip: {type(e).__name__}: {e}")


def parse_blend_from_zip_path(zip_path: str) -> dict:
    """Extract the first .blend file from a zip on disk and parse its frame range."""
    try:
...
def parse_upload_from_path(file_path: str, filename: str) -> dict:
    """Parse frame range from an uploaded file on disk (.blend or .zip)."""
    ext = Path(filename).suffix.lower()
    if ext == ".zip":
        return parse_blend_from_zip_path(file_path)
    elif ext == ".blend":
        return parse_blend_from_file(file_path)
    else:
        raise BlendParseError(f"Unsupported file extension: {ext}")
