...
Handles:
- Upsert one DB row per Modal endpoint
- Heartbeat daemon thread to keep machines visible
...
class MachineRegistrar:
    ...
    def register_all(self) -> list[str]:
        ...
        for ep in self._cfg.endpoints:
            ...
            if existing:
                ...
                log.info(f"Modal endpoint {ep.gpu_type} updated: {machine_id} ({ep.label})")
            else:
                ...
                log.info(f"Modal endpoint {ep.gpu_type} registered: {machine_id} ({ep.label})")
    ...
    def start_heartbeat(self) -> None:
        ...
        machine_keys = [
            f"modal-serverless-{ep.gpu_type}" for ep in self._cfg.endpoints
        ]
        ...
        t.start()
        log.info(f"Modal heartbeat thread started ({len(self._cfg.endpoints)} endpoint(s))")
