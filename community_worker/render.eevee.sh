#!/bin/bash
# render.eevee.sh — entry point for the community EEVEE worker image.
#
# Differences vs render.sh (the cycles entry point):
#   * Hardcodes EGL_PLATFORM=surfaceless (no DISPLAY, no Xvfb).  EEVEE on
#     a headless GPU needs NVIDIA's surfaceless EGL platform.  The
#     Dockerfile.eevee ships an NVIDIA EGL ICD JSON so GLVND's libEGL
#     dispatcher loads libEGL_nvidia.so.0 (when host-injected) instead
#     of falling back to Mesa.
#   * Logs whether libEGL_nvidia.so.0 is actually present so hosts that
#     fail to inject NVIDIA graphics caps are obvious from worker stdout.
#
# This script only ships in the EEVEE image.  Cycles renders go through
...
if ldconfig -p 2>/dev/null | grep -q "libEGL_nvidia.so.0"; then
    ...
    echo "         On Windows Docker Desktop + WSL2 this is a known limitation:"
    echo "         the NVIDIA Container Toolkit's 'graphics' capability has"
    echo "         limited WSL2 support.  Only Cycles is reliably available there."
fi
