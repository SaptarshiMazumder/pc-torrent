...
```bash
bash infra/scripts/push-worker.sh <env> 0.0.2 vast-cycles
bash infra/scripts/push-worker.sh <env> 0.0.2 vast-eevee
bash infra/scripts/push-worker.sh <env> 0.0.2 modal-cycles
bash infra/scripts/push-worker.sh <env> 0.0.2 community-cycles
```
Then on GHCR, flip each new `pc-rent-*-worker-*` package from private → **public** (Modal/Vast can't pull private images). Finally bump the four `*_IMAGE` lines in `.env` to `:<env>-v0.0.2`.
