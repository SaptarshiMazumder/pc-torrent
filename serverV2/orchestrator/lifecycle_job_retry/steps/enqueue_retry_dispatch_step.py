"""EnqueueRetryDispatchStep — manual-retry pipeline.

Builds the ``DispatchContext`` from the resolved ``RenderJob`` and
asks the coordinator to enqueue the retry task.  Stores the dispatch
results on ``ctx`` so the manual retry's result builder can return
``new_job_id`` to the API caller.

Manual retry's responsibility for unblocking dispatch on a failed
group lives entirely in the upstream
``ManualRetryFlipGroupPendingStep`` (writes group_status pending
before this step runs).  Auto-retry uses ``SubmitRetryStep`` instead;
the two paths share ``allocation_client.enqueue`` here.
